...
export default class TempleParser {
  ...
  //the scripts parser
  protected scriptParser: typeof Parser;
  //the symbol parser
  ...
  /**
   * Sets the code, setup the symbol and script parser
   */
  constructor(code: string) {
    this.code = code;
    this.symbolParser = new SymbolParser(code, this.walk.bind(this));
    this.scriptParser = acorn.Parser;
  }
    ...
    while (this.dataParser.optional('whitespace')) {
      ...
      if (hasValue) {
        ...
        properties.push({
          ...
          method: false,
          shorthand: false,
          computed: value.type === 'Identifier' || value.type === 'Program'
        });
      //<div show
      } else {
        //add this attribute to the properties
  ...
  protected close(symbol: Symbol) {
    ...
    if (type === 'ProgramExpression') {
      ...
      //parse the script 
      let source = this.symbolParser.substring(open.end, symbol.start);
      const parserConfig: acorn.Options = { ecmaVersion: 2020 };
      if (open.attributes) {
        const attributes = DataCompiler.object(open.attributes);
        if (attributes.type === 'module') {
          parserConfig.sourceType = 'module';
        } else if (attributes.type === 'text/typescript') {
          parserConfig.sourceType = 'module';
          source = ts.transpileModule(source, {}).outputText;
        }
      }
      const script: ScriptToken = {
        ...
        source: this.scriptParser.parse(source, parserConfig)
      };
  ...
  protected self(symbol: Symbol) {
    ...
    if (type === 'ImportDeclaration') {
      ...
      if (attributes) {
        ...
        const config = DataCompiler.object(attributes);
